...
def encode_locations(locations):
  ...
  for location in locations:
    points += [(location.location.lat, location.location.lon)]
    
  return encode_pairs(points)
...
def test_encode_locations():
    locations = (
       db.GeoPt(38.5, -120.2),
       db.GeoPt(40.7, -120.95),
       db.GeoPt(43.252, -126.453),
       db.GeoPt(40.7, -120.95),
    )
    expected_encoding = '_p~iF~ps|U_ulLnnqC_mqNvxq`@~lqNwxq`@', 'BBBB'
    assert encode_locations(locations) == expected_encoding
